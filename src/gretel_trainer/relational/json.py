...
def _normalize_json(nested_dfs, flat_dfs=None):
  ...
  dict_cols = [col for col in df.columns if df[col].apply(is_dict).any() and df[col].dropna().apply(is_dict).all()]
  list_cols = [col for col in df.columns if df[col].apply(is_list).any() and df[col].dropna().apply(is_list).all()]
  if dict_cols:
    ...
    for col in list_cols:
      ...
      nested_dfs.append((name + TABLE_SEPARATOR + col, new_table.reset_index(names=name + ID_SUFFIX)))
...
def denormalize_json(flat_tables):
  ...
  for table_name, table_df in table_dict.items():
    if table_df.empty and is_child_table(table_df):
      ...
      flat_df = table_df[col_names].rename(columns=dict(zip(col_names, new_col_names)))
      flat_dict = {k: {k1: v1 for k1, v1 in v.items() if v1 is not np.nan and v1 is not None} for k, v in flat_df.to_dict('index').items()}
      dict_df = nulls_to_empty_dicts(pd.DataFrame.from_dict({k: unflatten(v) for k, v in flat_dict.items()}, orient='index'))
      nested_df = table_df.join(dict_df).drop(columns=col_names)
      if is_child_table(nested_df):
        id_col = get_id_columns(nested_df)[0]
        parent_name = get_parent_table_name_from_child_id_column(id_col)
        nested_df = nested_df.sort_values(ORDER_COLUMN).groupby(id_col)[CONTENT_COLUMN].agg(list)
        col_name = get_parent_column_name_from_child_table_name(table_name)
        table_dict[parent_name] = table_dict[parent_name].join(nested_df.rename(col_name))
        table_dict[parent_name][col_name] = nulls_to_empty_lists(table_dict[parent_name][col_name])
...
class GretelJSON:
  ...
  def __init__(self, df, debug=False, **kwargs):
    ...
    for table_name, table_df in non_empty_tables:
      ...
      relational_data.add_table(name=table_name, primary_key=PRIMARY_KEY_COLUMN, data=table_df.reset_index())

    for table_name, table_df in non_empty_tables:
      for column in get_id_columns(table_df):
        fk = table_name + '.' + column
        ref = self.table_name_mappings[get_parent_table_name_from_child_id_column(column)] + '.' + PRIMARY_KEY_COLUMN
  ...
  def get_generated_tables(self):
    output_tables = []
    inverse_table_name_mappings = {value: key for key, value in self.table_name_mappings.items()}
    for t in self.tables:
      empty_table = self.empty_tables.get(t)
      multitable_output = self.multitable.synthetic_output_tables.get(t, empty_table)
  ...
  def save_jsonl(self, filename):
    return self.get_generated_tables().to_json(filename, orient='records', lines=True)

  def display_report(self):
    import IPython
    from smart_open import open
    report_path = str(self.multitable._working_dir / self.multitable._synthetics_run.identifier / "relational_report.html")
