...
def test_ignores_preserved_tables(pets, tmpdir):
    ...
    task.each_iteration()
    assert "pets" not in task.record_handlers
...
def test_starts_jobs_for_ready_tables(pets, tmpdir):
    ...
    assert len(task.record_handlers) == 0

    task.each_iteration()

    assert len(task.record_handlers) == 1
    assert "humans" in task.record_handlers
    task.models["humans"].create_record_handler_obj.assert_called_once()
    task.record_handlers["humans"].submit_cloud.assert_called_once()
...
def test_defers_jobs_if_no_room(pets, tmpdir):
    ...
    assert "humans" in task.record_handlers
    task.models["humans"].create_record_handler_obj.assert_called_once()
    task.record_handlers["humans"].submit_cloud.assert_not_called()
...
def test_does_not_restart_existing_deferred_jobs(pets, tmpdir):
    ...
    task.models["humans"].reset_mock()

    # On next iteration, we do not *re-create* a record handler object for humans
    task.each_iteration()
    task.models["humans"].create_record_handler_obj.assert_not_called()
